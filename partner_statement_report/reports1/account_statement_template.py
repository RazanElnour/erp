...
class AccountStatementReport(models.AbstractModel):
    _name = 'report.is_penotee_accounting.account_statement_pdf_report'
        ...
        if data:
            if data['from_date'] and data['to_date']:
                move_line_ids = self.env['account.move.line'].search(
                    [('date', '<=', data['to_date']), ('date', '>=', data['from_date']), ('move_id.state', '=', 'posted'),
                ...
                if move_line_ids:
                    ...
                    for move in records:
                        if data['currency'] == False:
                            ...
                            res.append({'date': date, 'name': name, 'journal_id': journal_id, 'move_id': move_id,
                                        'i_debit': i_debit, 'i_credit': i_credit})
                        # if data['currency'] == False:
                        #     i_debit = move.debit
                        #     i_credit = move.credit
                        elif data['currency'] == True:
                            if move.currency_id:
                                if move.currency_id.name != 'SDG':
                                    # raise UserError((move.move_id.id))
                                    date = move.date
                                    name = move.name
                                    journal_id = move.journal_id.code
                                    move_id = move.move_id.name
                    ...
                    # res.append({'i_debit': i_debit, 'i_credit': i_credit})
                                    res.append({'date': date, 'name': name, 'journal_id': journal_id, 'move_id': move_id, 'i_debit': i_debit, 'i_credit': i_credit})
    ...
    def _get_report_values(self, docids, data=None):
        ...
        docs = data['records']
        sales_details_report = self.env['ir.actions.report']._get_report_from_name('is_penotee_accounting.account_statement_pdf_report')
        docargs = {
